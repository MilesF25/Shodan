...
def save_csvname(filename="devices_info.csv"):
    ans = input(
        "Would you like to name the csv something other than devices_info.csv? yes/no: "
    )

    if ans.lower() == "yes" or ans.lower() == "y":
        filename = input("Enter your desired filename (with .csv extension): ").strip()

        if not filename.endswith(".csv"):
            filename += ".csv"
    else:
        filename = "devices_info.csv"
